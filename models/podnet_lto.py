...
from utils.toolkit import tensor2numpy, calibratedCE, ForgetAwareAWP
...
class PODNet_lto(BaseLearner):
    def __init__(self, args):
        super().__init__(args)
        self._network = CosineIncrementalNet(args.convnet_type, args.method, pretrained=False, nb_proxy=nb_proxy)
    ...
    def _train(self, data_manager, train_loader, test_loader):
        ...
        optimizer = ForgetAwareAWP(network_params, torch.optim.SGD, lr=lrate, momentum=0.9,
    ...
    def _run(self, train_loader, test_loader, optimizer, scheduler, epk):
        for epoch in range(1, epk+1):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                if self._old_network is not None:
                    criterion = calibratedCE(self.cls_num_list, self._known_classes, self._total_classes).to(self._device)
                    lsc_loss = criterion(logits, targets, device=self._device)
                else:
                    lsc_loss = F.cross_entropy(logits, targets)  # Cross entropy loss
