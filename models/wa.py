...
class WA(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        test_dset_new = data_manager.get_dataset(np.arange(self._known_classes, self._total_classes), source='test',
                                                 mode='test')
        ...
        for k in range(self._total_classes - self._known_classes):
            m_k = round((1 + 1.5*(acc_mean - acc_old_list[k]) / acc_mean) * self.samples_per_class)
            m_list.append(m_k)
        print(m_list)
