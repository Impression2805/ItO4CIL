...
from convs.resnet18_cifar import resnet18_cifar
...
def get_convnet(convnet_type, pretrained=False):
    ...
    if name == 'resnet32':
        ...
        return resnet18_cifar(pretrained=pretrained)
    elif name == 'resnet34':
...
class CosineIncrementalNet(BaseNet):
    def __init__(self, convnet_type, method, pretrained, nb_proxy=1):
        super().__init__(convnet_type, pretrained)
    ...
    def update_fc(self, nb_classes, task_num):
        if self.method == 'classaug':
            fc = self.generate_fc(self.feature_dim, 4 * nb_classes)
        else:
            fc = self.generate_fc(self.feature_dim, nb_classes)
        if self.fc is not None:
            if task_num == 1:
